    ...
    const { navigate, isUserLoggedIn} = useMainStore()
      ...
      return (
            <aside className={`hidden border-l border-neutral-400/50 pt-4 px-4 sticky top-0 h-screen shrink-0 max-w-[22rem] overflow-y-auto scrollbar-hide md:block ${className}`}>
              ...
              <a href="https://github.com/andrei0x309/fc-app.pages.dev" className="flex content-end items-end text-neutral-600 dark:text-neutral-400 -mt-2 text-[0.77rem] text-right">View the source code on GitHub<GithubIcon className="h-4 w-4 ml-1 mr-1 inline" /></a>
              {process.env.GIT_HASH_COMMIT
              && <a href={`${process.env.GIT_HASH_COMMIT?.includes('development') ? '#' : `https://github.com/andrei0x309/fc-app.pages.dev/commit/${process.env.GIT_HASH_COMMIT}`}`} className="flex content-end items-end text-neutral-600 dark:text-neutral-400 mt-1 text-[0.77rem] text-right">
