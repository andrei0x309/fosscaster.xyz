        ...
        <div className={`min-h-screen text-white ${className}`}>
          <div className="h-full min-h-screen">
            ...
            <div className="p-4 h-full min-h-screen">
              ...
              {activeTab === 'users' && (
                <>
                  {!isUsersFirstLoad && loading ? <SimpleLoader /> : null}

                  {users?.result?.users.map((user, index) => (
                    <UserItem key={index} user={user} />
                  ))}
                  {loading && <SimpleLoader />}
                  {isUsersFirstLoad && <InfiniteScroll hasMore={hasMoreUsers} isLoading={loading} next={loadMoreUsers} threshold={0.3}>
                    <div className='my-2'>
                    </div>
                  </InfiniteScroll>
                  }
              ...
              {activeTab === 'channels' && (
                <>
                  ...
                  {!isChannelsFirstLoad && loading ? <SimpleLoader /> : null}
                  ...
                  {loading && <SimpleLoader />}
                  {isChannelsFirstLoad && <InfiniteScroll hasMore={hasMoreChannels} isLoading={loading} next={loadMoreChannels} threshold={0.3}>
